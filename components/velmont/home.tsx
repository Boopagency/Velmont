...
const CTA = ({ children = 'Solicitar uma análise estratégica', className = '' }: { children?: React.ReactNode; className?: string }) => <a className={`cta ${className}`} href={whatsappUrl()} target="_blank" rel="noopener noreferrer"><span>{children}</span><span className="cta-arrow"><Arrow /></span></a>;
...
export function Header({ inner = false }: { inner?: boolean }) {
  ...
  return <>{inner && <a className="skip-link" href="#inicio">Pular para o conteúdo</a>}<header className={`site-header ${inner ? 'inner-header' : ''}`}>
    ...
    <nav className="desktop-nav" aria-label="Navegação principal"><a href={`${root}#atuacao`}>Serviços</a><a href={`${root}#essencia`}>Nossa essência</a><a href={`${root}#processo`}>Como atuamos</a><a href={`${root}#insights`}>Insights</a></nav>
    <a className="header-contact" href={whatsappUrl()} target="_blank" rel="noopener noreferrer">Vamos conversar <Arrow /></a>
    ...
    {menu && <nav id="mobile-menu" className="mobile-menu" aria-label="Navegação mobile">{[['Serviços','atuacao'],['Nossa essência','essencia'],['Como atuamos','processo'],['Insights','insights'],['Solicitar análise','contato']].map(([label,id],i) => <a key={id} href={id === 'contato' ? whatsappUrl() : `${root}#${id}`} target={id === 'contato' ? '_blank' : undefined} rel={id === 'contato' ? 'noopener noreferrer' : undefined} onClick={() => setMenu(false)}><small>0{i+1}</small>{label}<Arrow /></a>)}</nav>}
  </header></>;
}
...
export function Home({ posts }: { posts: BlogSummary[] }) {
  return <><a className="skip-link" href="#conteudo">Pular para o conteúdo</a><Motion />
    ...
    <main id="conteudo">
      ...
      <section id="atuacao" className="services wrap"><div className="section-heading"><Tag n="02">O QUE PROTEGEMOS</Tag><h2 data-reveal>Seu próximo passo.<br /><em>Nossa visão estratégica.</em></h2></div>
        <article className="service service-brand" id="marcas"><div className="service-art" data-scene><BrandPerimeter /><img src="/generated/velmont-trademark-bottle-1192.webp" srcSet="/generated/velmont-trademark-bottle-640.webp 640w, /generated/velmont-trademark-bottle-1192.webp 1192w" sizes="(max-width: 767px) 100vw, 50vw" width="1192" height="1320" alt="Frasco com o nome Velmont delimitado por uma moldura dourada, representação de proteção e exclusividade da marca" loading="lazy" /><span className="art-caption">SUA MARCA. UM ATIVO A PROTEGER.</span><span className="art-number" aria-hidden="true">01</span></div><div className="service-copy" data-reveal><span className="eyebrow">MARCAS</span><h3>O nome é seu.<br /><em>E o direito de usá-lo?</em></h3><p>Sua marca carrega reputação, investimento e história. O primeiro passo é entender o que pode ser protegido e quais riscos precisam ser considerados.</p><ul className="service-list"><li>Pesquisa de anterioridade e viabilidade</li><li>Pedido de registro e estratégia de proteção</li><li>Monitoramento e acompanhamento</li></ul><CTA>Vamos falar sobre sua marca</CTA><a className="service-related" href="/blog/antes-de-registrar-sua-marca">Leia: o que analisar antes de solicitar o pedido de registro de uma marca</a><p className="service-note">Toda solicitação está sujeita à análise do INPI. Clareza desde o início.</p></div></article>
        <article className="service service-patent" id="patentes"><div className="service-copy" data-reveal><span className="eyebrow">PATENTES & DESENHO INDUSTRIAL</span><h3>Você vê uma invenção.<br /><em>Nós vemos um ativo.</em></h3><p>Uma solução técnica, inovadora ou design exclusivo podem representar uma vantagem importante para o negócio. Investigamos possibilidades de proteção antes de traçar o caminho.</p><ul className="service-list"><li>Busca de anterioridade e análise patentária</li><li>Patentes de invenção e modelos de utilidade</li><li>Desenho industrial e design exclusivo com estratégia de proteção</li></ul><a className="text-link" href={whatsappUrl()} target="_blank" rel="noopener noreferrer">Converse sobre sua criação <Arrow /></a><a className="service-related" href="/blog/inovacao-e-patente">Leia: da inovação à proteção por patente</a></div><PatentScene /></article>
        <article className="software" id="software"><div className="software-top"><span className="eyebrow">03 / SOFTWARE E TECNOLOGIA, DIREITO AUTORAL E PROVA DE ANTERIORIDADE</span><span className="software-tag">O DIGITAL TAMBÉM É PATRIMÔNIO.</span></div><div className="software-grid"><h3>Não é só código.<br />É <em>valor construído.</em></h3><div><p>Seu software reúne conhecimento, tempo e investimento. A proteção também passa por autoria, titularidade e pelas relações de quem cria com você.</p><a className="text-link" href={whatsappUrl()} target="_blank" rel="noopener noreferrer">Proteja seu ativo digital <Arrow /></a><a className="service-related" href="/blog/software-tambem-e-patrimonio">Leia: registro de software, autoria e titularidade</a></div></div><div className="software-asset"><SoftwareScene /><div className="software-layers" aria-label="Frentes de proteção do software"><div><span>01</span><strong>Código</strong><p>Registro de programa de computador</p></div><div><span>02</span><strong>Autoria</strong><p>Proteção do autor e anterioridade da existência da documentação de quem desenvolve</p></div><div><span>03</span><strong>Titularidade</strong><p>Contratos, cessão e licenciamento</p></div></div></div></article>
        <div className="other-services"><span>UM OLHAR INTEGRADO</span><p>Direitos autorais <i>/</i> Contratos e titularidade <i>/</i> Consultoria empresarial <i>/</i> Naming & identidade visual</p><a href={whatsappUrl()} target="_blank" rel="noopener noreferrer" aria-label="Conversar sobre outras frentes de atuação"><Arrow /></a></div>
      </section>
